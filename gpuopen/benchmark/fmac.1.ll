...
define float @__rocm_fmaf(float %a, float %b, float %c) {
  %1 = tail call float asm sideeffect "v_mov_b32 $0, $1","=v,v"(float %c)
  tail call void asm sideeffect "v_mac_f32 $0, $1, $2","v,v,v"(float %1, float %a, float %b)
;  %c = tail call float asm sideeffect "v_mac_f32 $0, $1, $2","=v,v,v"(float %a, float %b)
  ret float %1
}
